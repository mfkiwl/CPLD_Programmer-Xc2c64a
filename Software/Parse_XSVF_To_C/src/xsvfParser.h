...
protected:
    ...
    */
   bool parseAll(unsigned xsvf_data_size);
...
class XsvfParser_Array : public XsvfParser {
...
public:
   /**
    * Constructor
    * Create a XSVF parser based on an array
    *
    * @param fileName         Name of file for output
    * @param xsvf_data_size   Size of XSVF data
    * @param xsvf_data        Data array for XSVF sequence to play
    */
   XsvfParser_Array(const char *fileName, unsigned xsvf_data_size, const uint8_t *xsvf_data) :
      XsvfParser(fileName), xsvf_data_size(xsvf_data_size), xsvf_data(xsvf_data) {
   }

   ~XsvfParser_Array(){}

   bool parseAll() {
      return XsvfParser::parseAll(xsvf_data_size);
   }
};

class XsvfParser_File : public XsvfParser {
...
public:
    ...
    * Constructor
    */
   XsvfParser_File(const char *fileName) : XsvfParser(fileName) {
   }
   ...
   bool parseAll() {
      ...
      if (!XsvfParser::parseAll(xsvf_data_size)) {
         return false;
      }
